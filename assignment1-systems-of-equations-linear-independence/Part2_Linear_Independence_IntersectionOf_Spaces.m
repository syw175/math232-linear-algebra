...
sprintf('Part 2A: Show B1 is a linearly dependent set.')
% Create a system of equations using the vectors in B1
B1 = [w1(:), w2(:), w3(:), w4(:), w5(:), w6(:)];
% Initialize a zero vector to form augmented matrices 
zeroVectorR5 = [0; 0; 0; 0; 0];

% Create an augmented matrix wih the zero vector and row reduce it to RREF
...
sprintf(['The maximal linearly independent set B''1 of vectors is {w1, w2, w3, w4}' ...
    'as seen in the RREF x5 and x6 do not have leading 1s'])
...
sprintf('Show that vectors from B1 that are NOT in B′1 set are contained in the span of B′1 (and hence, that span B1 = span B′1).')
...
% Create an augmented matrix wih the zero vector and row reduce to RREF
...
% I am using the maximal linearly independent sets of B1 and B2
% Set the linearly combination of both spans equal to each other.
...
% Convert this with equations?? Somehow?
